...
export default function TheaterZoneSelector({
  ...
  const ZONES = useMemo(
    () => ({
      ...
      tribuna_oeste: { id: "tribuna_oeste", label: "Tribuna Oeste", color: "#2563eb" },
      tribuna_este:  { id: "tribuna_este",  label: "Tribuna Este",  color: "#2563eb" },
      planta_baja:   { id: "planta_baja",   label: "Planta Baja",   color: "#22c55e" },
    }),
    []
  );
      ...
      <div className="rounded-xl border p-3">
        ...
        >
          <defs>
            ...
            <pattern id="diag-disabled" width="8" height="8" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
          ...
          <g
            ...
            onClick={!readOnlyEscenario ? () => applyModeTo("escenario") : undefined}
            onKeyDown={!readOnlyEscenario ? (e) => {
              if (e.key === "Enter" || e.key === " ") { e.preventDefault(); applyModeTo("escenario"); }
            } : undefined}
            cursor={readOnlyEscenario ? "default" : "pointer"}
          >
            <ellipse cx="400" cy="120" rx="230" ry="95" fill="none" stroke={strokeFor("escenario")} strokeWidth="6" />
            <text x="400" y="125" textAnchor="middle" fontSize="28" fontWeight="600" fill={strokeFor("escenario")}>
...
function ZoneOval({ id, state, labelLines, cx, cy, rx, ry, fill, stroke, amber, onActivate }) {
  ...
  return (
    <g
      ...
      cursor="pointer"
    >
      <ellipse cx={cx} cy={cy} rx={rx} ry={ry} fill={fill} stroke={stroke} strokeWidth="4" />
      {isBlocked && (
        <ellipse cx={cx} cy={cy} rx={rx} ry={ry} fill="url(#diag-disabled)" opacity={0.45} />
      )}
      {isSelected && (
        <ellipse cx={cx} cy={cy} rx={rx - 6} ry={ry - 6} fill="none" stroke={amber} strokeWidth="6" />
      )}
      <text x={cx} y={cy - 5} textAnchor="middle" fontSize="20" fontWeight="600" fill="#ffffff">
        {labelLines[0]}
      </text>
      <text x={cx} y={cy + 20} textAnchor="middle" fontSize="20" fontWeight="600" fill="#ffffff">
        {labelLines[1]}
      </text>
    </g>
  );
...
function ZoneRect({ id, state, label, x, y, w, h, rx, fill, stroke, amber, onActivate }) {
  const isBlocked   = state === "blocked";
  const isSelected  = state === "selected";
  return (
    <g
      ...
      cursor="pointer"
    >
      <rect x={x} y={y} width={w} height={h} rx={rx} fill={fill} stroke={stroke} strokeWidth="4" />
      {isBlocked && (
        <rect x={x} y={y} width={w} height={h} rx={rx} fill="url(#diag-disabled)" opacity={0.45} />
      )}
      {isSelected && (
        <rect x={x + 6} y={y + 6} width={w - 12} height={h - 12} rx={rx} fill="none" stroke={amber} strokeWidth="6" />
      )}
      <text
        x={x + w / 2}
        y={y + h / 2}
